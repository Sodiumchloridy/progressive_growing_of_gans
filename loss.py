...
def G_wgan_acgan(G, D, opt, training_set, minibatch_size,
    ...
    if D.output_shapes[1][1] > 0:
        with tf.name_scope('LabelPenalty'):
            label_penalty_fakes = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=fake_labels_out)
...
def D_wgangp_acgan(G, D, opt, training_set, minibatch_size, reals, labels,
    ...
    if D.output_shapes[1][1] > 0:
        with tf.name_scope('LabelPenalty'):
            label_penalty_reals = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=real_labels_out)
            label_penalty_fakes = tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=fake_labels_out)
